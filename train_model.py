    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            with torch.amp.autocast('cuda'):
                ...
                # Calculate losses
                direction_loss = direction_loss_fn(outputs['direction_logits'], direction_targets)
                volatility_loss = regression_loss_fn(outputs['volatility'], volatility_targets)
                price_change_loss = regression_loss_fn(outputs['price_change'], price_change_targets)
                spread_loss = regression_loss_fn(outputs['spread'], spread_targets)
        ...
        if val_score > best_val_score:
            ...
            logger.info(f"No improvement for {no_improve_counter} epochs")
...
def validate_model(
    ...
    # Only consider samples with significant price movement for direction metrics
    price_change_mask = []
    ...
    with torch.no_grad():
        for inputs, targets in val_loader:
            ...
            # Weighted total loss
            total_loss += (
                direction_weight * direction_loss.item() + 
            ...
            # Store price change mask
            abs_price_change = torch.abs(price_change_targets.cpu())
            price_change_mask.extend((abs_price_change >= min_price_change).numpy())
    ...
    price_change_mask = np.array(price_change_mask)
    
    # Only evaluate direction predictions for samples with significant price movement
    significant_indices = np.where(price_change_mask)[0]
    
    if len(significant_indices) > 0:
        filtered_preds = all_direction_preds[significant_indices]
        filtered_targets = all_direction_targets[significant_indices]
        
        # Calculate metrics
        accuracy = accuracy_score(filtered_targets, filtered_preds)
        precision = precision_score(filtered_targets, filtered_preds, zero_division=0)
        recall = recall_score(filtered_targets, filtered_preds, zero_division=0)
        f1 = f1_score(filtered_targets, filtered_preds, zero_division=0)
    else:
        # If no significant price movements, set metrics to 0
