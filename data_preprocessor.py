...
class FinancialDataPreprocessor:
    ...
    def _create_sequences(self, data, window_size, horizon):
        ...
        normalized_features = self.hourly_scaler.fit_transform(data[feature_cols].values)
        normalized_data = pd.DataFrame(normalized_features, index=data.index, columns=feature_cols)
        
        for col in target_cols:
            normalized_data[col] = data[col]
        
        for i in range(len(normalized_data) - window_size - horizon + 1):
            window_data = normalized_data.iloc[i:i+window_size][feature_cols].values
            target_idx = i + window_size + horizon - 1
            
            if np.isnan(window_data).any() or target_idx >= len(normalized_data):
                continue
                
            X.append(window_data)
            
            y.append([
                normalized_data.iloc[target_idx]['IsUp'],
                normalized_data.iloc[target_idx]['Volatility'],
                normalized_data.iloc[target_idx]['PriceChangePercent'],
                normalized_data.iloc[target_idx]['SpreadEstimate']
            ])
